...
static sfVector2f sec_move_model(sfFloatRect tmp, sfFloatRect tmp2,
...
enemies_t *enemy)
{
    ...
    && (tmp.left - tmp2.left >= tmp2.top - tmp.top)) {
        pos.x += 45;
        temp_rect = (sfIntRect){0, 288, 24, 48};
    }
    if (tmp.left < tmp2.left && (tmp2.left - tmp.left >= tmp.top - tmp2.top)
    && (tmp2.left - tmp.left >= tmp2.top - tmp.top)) {
        pos.x -= 45;
        temp_rect = (sfIntRect){0, 338, 24, 48};
    }
    pos = sec_move_model(tmp, tmp2, pos, &temp_rect);
